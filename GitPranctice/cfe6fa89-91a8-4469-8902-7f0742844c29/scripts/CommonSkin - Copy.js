		 ...
		 if (objType == "content object" || objType == "contentobject"){
			 if( trackObjects[SeqID].chkcontraintfornavigation != "no") {
				...
				if (isinstancycontent == true) {
			     ...
			     if (getCoValues("Qtype").toLowerCase() == "longanswer" && (isRequriedAttachLA || checkForVideoCapture() || checkForImageCapture())) {
					if (checkAnswers("next") == false )
					    return;
					if (isinstancycontent == true) {
					    SaveQuestionData();//13-07-2016 :  Bhushanam added this code for update the each question data to DB.
...
function checkAnswer() {
    ...
    try {
        ...
        // Added by maheedhar for singlepagerendering in co on 16-10-2014
        if (trackObjects[SeqID].singleqtnperpage == "no" && isLongAnswer) {
            isNoDocAttached = chkContentAttached();
        }
        if (trackObjects[SeqID].singleqtnperpage == "no" && ( (!isNoDocAttached) || !(validateCaptureVideo()) || !(validateCaptureImage()) )) {
			...
			if (tincan.recordStores.length > 0)
			    ...
			    try {
                        ...
                        if (pages[SeqID][i].Qtype == "draganddrop" || pages[SeqID][i].Qtype == "sequence") {
                            ...
                            pages[SeqID][i].useranswer = prearr[k].split(aSeperator)[1];
                                // Added for Mobile Native..By Satish
                                if (nativeCID != "" && pages[SeqID][i].Qtype == "sequence") {
                                    pages[SeqID][i].useranswer = ReplaceAll(pages[SeqID][i].useranswer, '%5E', '^');
                                }//
				...
				if (tincan.recordStores.length > 0)
				    ...
				    getAttachedContentData(pages[SeqID][i].pages[m], prearr[k], i);
                                ...
                                if (pages[SeqID][i].pages[m].Qtype == "draganddrop" || pages[SeqID][i].pages[m].Qtype == "sequence") {
                                    ...
                                    pages[SeqID][i].pages[m].useranswer = prearr[k].split(aSeperator)[1];
                                    // Added for Mobile Native..By Satish
                                    if (nativeCID != "" && pages[SeqID][i].pages[m].Qtype == "sequence") {
                                        pages[SeqID][i].pages[m].useranswer = ReplaceAll(pages[SeqID][i].pages[m].useranswer, '%5E', '^');
                                    }//
                                }
                        break;
					...
					if (tincan.recordStores.length > 0)
					    ...
					    getAttachedContentData(pages[SeqID][i].pages[m].pages[n], prearr[k], i);
                                        ...
                                        if (pages[SeqID][i].pages[m].pages[n].Qtype == "draganddrop" || pages[SeqID][i].pages[m].pages[n].Qtype == "sequence") {
                                            ...
                                            // Added for Mobile Native..By Satish
                                            if (nativeCID != "" && pages[SeqID][i].pages[m].pages[n].Qtype == "sequence") {
	...
	var data1 = data;
	  var sampleDiv = document.createElement("div");
		$(sampleDiv).html(data1);
         $(sampleDiv).find("table#instmaintable div").each(function (i){			 
...
function showHintLayer(obj) {
    var hasContent = chkSoultionHintLayerHasContent(obj, "hints");
...
function generateHintPopup(obj){
 var hintLayerDiv;
    if (trackObjects[SeqID].singleqtnperpage == "no") {
        ...
        var instTblId = "#instmaintable" + hintPageId
...
function showSolutionLayer(obj) {
    var hasContent = chkSoultionHintLayerHasContent(obj, "solution");
    if (hasContent == false) { return false };
...
function chkSoultionHintLayerHasContent(obj, type) {
    ...
    if (trackObjects[SeqID].singleqtnperpage == "yes") {
        ...
        var instTbl = "#instmaintable" + objId;
...
function disableSolutionButton() {
    if (trackObjects[SeqID].singleqtnperpage == "yes") {
        if (document.getElementById("solutionbut") != null || document.getElementById("solutionbut") != undefined) {
            if (document.getElementById("submitbut") != null || document.getElementById("submitbut") != undefined) {
                $('#solutionbut').unbind('click');
                $('#solutionbut').css({ "opacity": "0.5", "cursor": "default" });
...
function enableSolutionButton() {
    if (trackObjects[SeqID].singleqtnperpage == "yes") {
        if (document.getElementById("solutionbut") != null || document.getElementById("solutionbut") != undefined) {
            $('#solutionbut').unbind('click');
            $('#solutionbut').bind('click', function () { showSolutionLayer($(this)) });
            $('#solutionbut').css({ "opacity": "1", "cursor": "pointer" });
        }
    }

}
function showHideHintButton() {   
    try{
		$("#page").find("img[id^='hintbut']").each(function(e){
			var hasContent = chkSoultionHintLayerHasContent($(this), "hints");
			if (hasContent == false) { $(this).hide(); }
			else {$(this).show();}
		})	
	}catch(e){}
}
function showHideSolutionButton() { 
    try {

		$("#page").find("img[id^='solutionbut']").each(function(e){
			var hasContent = chkSoultionHintLayerHasContent($(this), "solution");
			if (hasContent == false) { $(this).hide(); }
			//else {$(this).show();}
		})
...
function chkIsInstancyContent() {
    try{
        if (window.location.href.indexOf("IsInstancyContent=true") > -1 || window.location.href.indexOf('mode=preview') > -1) {
            return true;
        }
        else {
            return false;
...
$(function () {
    debugger;
    $(".SectionDiv").scroll(function (event) {

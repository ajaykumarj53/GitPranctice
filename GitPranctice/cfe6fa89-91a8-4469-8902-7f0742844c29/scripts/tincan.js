        ...
        */
        sendRequest: function (cfg) {
            ...
            if (this._requestMode === NATIVE) {
                ...
                //--INST-2271-- Added for Xapi offline tracking by krishna B.
                if (fnGetQueryString("cid") != "") {
                    try {
                        if (cfg.method == "GET")
                            return null;
                        MobileJSInterface.XHR_requestWithLrsUrlMethodDataAuthCallbackIgnore404(this.endpoint, fullUrl, cfg.method, cfg.data, this.auth, "", "", "", "");
                        return;
                    }
                    catch (e) {                     
                    }
                }
                if (fnGetQueryString("cid") == "" && window.location.href.indexOf("nativeappURL") != -1) {
                    try {
                        if (cfg.method != "GET")
                            MobileJSInterface.XHR_requestWithLrsUrlMethodDataAuthCallbackIgnore404(this.endpoint, fullUrl, cfg.method, cfg.data, this.auth, "", "", "", "");
                    }

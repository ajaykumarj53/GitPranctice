	...
	$.fn.speedometer = function( options ){
		...
		var def = {
			...
			thisCss: {
				...
				fontFamily: 'Arial',
				fontWeight: '900',
				backgroundImage: "url('Images/speedometer.jpg')"
		...
		if ( def.limit && ( def.percentage > def.maximum || def.percentage < def.minimum ) ) {
			/* the glass cracks */
		    $this.css('backgroundImage', 'url("Images/speedometer.jpg")');
		} else {
			...
			$this.jqcanvas ( function ( canvas, width, height ) {
				...
				function animate(){		
					...
					ctx.rotate(( i * Math.PI / def.scale )/4);
		...
		digitalGauge.attr("id","metercenter")

...
NativeAPIAdapter = function() {
        ...
        try {
            ...
            switch (name.toLowerCase()) {
                ...
                case "instancy.textresponses":
                    return AndroidLRSFunction.GetUserTextResponses(SeqID, ctOBj.uid);
                    break;
                default:
                    break;
            }
    ...
    NativeAPIAdapter.prototype.LMSCommit = function(value) {
        try {
            ...
            AndroidLRSFunction.UpdateUserTextResponses(SeqID, ctOBj.uid, ctOBj.text_responses);
        } catch (e) {
    ...
    NativeAPIAdapter.prototype.LMSTrackInitialize = function(value) {
        try {
            ...
            if (ctOBj != null) {
                ...
                ctOBj.location = fnGetDataBySeqId(ctOBj.trackbookmark, strLOc);
                ctOBj.status = fnGetQueryString("lstatus");
                var strsuspenddata = fnGetQueryString("susdata");
                ctOBj.suspend_data = ReplaceAll(strsuspenddata, '%23', '#');                
                ctOBj.question_data = ReplaceAll(fnGetQueryString("quesdata"), '%20', ' ');
                ...
                document.getElementById("hdnQuseData").value = ReplaceAll(fnGetQueryString("LtQuesData"), '%20', ' ');
                document.getElementById("hdnQuseData").value = ReplaceAll(document.getElementById("hdnQuseData").value, '%23', '#');
                ...
                ctOBj.suspend_data = fnGetDataBySeqId(SeqID, ctOBj.track_suspenddata);
                var strname = fnGetQueryString("sname");
                ctOBj.sname = ReplaceAll(strname, '%20', ' ');
            }
            return "";
    ...
    NativeAPIAdapter.prototype.LMSTrackGetValue = function(name, id) {
        try {
            ...
            switch (name.toLowerCase()) {
                ...
                case "cmi.core.lesson_status":
                    //return ctOBj.status
                    return fnGetDataBySeqId(id, ctOBj.trackstatus);
                    break;
                case "cmi.core.lesson_location":
                    return ctOBj.location;
                    break;
                ...
                case "instancy.suspend_data":
                    return fnGetDataBySeqId(id, ctOBj.track_suspenddata);
                    break;
                default:
                    break;
            }
...
function fnGetDataBySeqId(index, valueString) {
    var vars = valueString.split("@");
    try {
        for (var i = 0; i < vars.length; i++) {
...
function CourseTrackingObj(cid, uid, sname, status, location, sestime, totaltime, suspend_data, question_data, scoreraw, trackbookmark, trackstatus, track_suspenddata,text_responses) {
    this.cid = cid;
    this.uid = uid;
    this.sname = sname;
    this.status = status;
